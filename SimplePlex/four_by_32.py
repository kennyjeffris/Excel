...
def summary_3(wb, analytes, headerLists, searchLists, max_row, max_col):
    ...
    ws4['A36'].value = headerList5[0]
    ws4['A37'].border = style['medium']
    for index, col in enumerate(iterable=ws4.iter_cols(min_row=37,
                                                       min_col=2, max_row=37, max_col=5)):
        for cell in col:
            cell.value = headerList3[index + 1]
            cell.border = style['medium']

    for index, row in enumerate(iterable=ws4.iter_rows(min_row=38, max_row=42,
                                                       min_col=1, max_col=1)):
        for cell in row:
    ...
    for index, col in enumerate(iterable=ws4.iter_cols(
        ...
        for index2, row in enumerate(iterable=ws4.iter_rows(
                range_string='{}38:{}42'.format(let,
                                                let))):
            for cell in row:
                coeffs.append(cell.value)
        xvalues = []
        values = []
        xvalues.extend((.1, 1, 10, 100, 1000, 10000, 100000))
        ...
        for index4, row in enumerate(iterable=ws4.iter_rows(
                range_string='{}44:{}122'.format(let, let))):
            for cell in row:
        ...
        xref = Reference(ws4, min_col=2 + index, max_col=2 + index,
                         min_row=44, max_row=43 + len(xvalues))
        yref = Reference(ws4, min_col=2 + index, max_col=2 + index,
                         min_row=44 + len(xvalues),
                         max_row=43 + len(xvalues) + len(values))
        ...
        chart.x_axis.scaling.max = 100000
        chart.y_axis.scaling.max = 100000
