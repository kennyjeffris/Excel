...
def summary_3(wb, analytes, headerLists, searchLists, max_row, max_col):
    ...
    for index, col in enumerate(iterable=ws4.iter_cols(
        ...
        values = get_items(ws1, headerList4[0], max_row, max_col)
    ...
    ws4['A76'].value = headerList5[0]
    ws4['A77'].border = style['medium']
    for index, col in enumerate(iterable=ws4.iter_cols(min_row=77,
                                                       min_col=2, max_row=77, max_col=2)):
        for cell in col:
            cell.value = headerList3[index + 1]
            cell.border = style['medium']

    for index, row in enumerate(iterable=ws4.iter_rows(min_row=78, max_row=82,
                                                       min_col=1, max_col=1)):
        for cell in row:
    ...
    for index, col in enumerate(iterable=ws4.iter_cols(
            min_col=2, max_col=len(headerList3),
            min_row=78, max_row=82)):

        for index2, row in enumerate(iterable=ws4.iter_rows(
                min_col=index + 2,
                min_row=78,
                max_row=82)):
            values = get_items(ws1, headerList4[index2 + 1], max_row, max_col)
            for cell in row:
                cell.value = values[0]
                cell.border = style['thin']
    ...
    for index, col in enumerate(iterable=ws4.iter_cols(
        ...
        for index4, row in enumerate(iterable=ws4.iter_rows(
                range_string='{}84:{}240'.format(let, let))):
            for cell in row:
        ...
        xref = Reference(ws4, min_col=2 + index, max_col=2 + index,
                         min_row=84, max_row=83 + len(xvalues))
        yref = Reference(ws4, min_col=2 + index, max_col=2 + index,
                         min_row=84 + len(xvalues),
                         max_row=83 + len(xvalues) + len(values))
        ...
        ws4.add_chart(chart, 'D{}'.format((index * 15) + 1))
    return wb